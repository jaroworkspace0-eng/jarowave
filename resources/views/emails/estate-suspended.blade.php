...
<p style="text-align:center; font-size:14px; color:#888; margin:0 0 28px;">
    @if($neverActivated)
        ...
        the first bulk billing payment for this estate was not received within the 7-day grace period,
        so Echo Link protection was never activated for opted-in households.
    @else
        Hi {{ $billingContact->name }},
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin:24px 0;">
    ...
    <tr>
        ...
        <td style="padding:4px 0; font-size:13px; font-weight:700; color:#ef4444; text-align:right;">
            {{ $neverActivated ? 'Never Activated' : 'Suspended' }}
        </td>
    </tr>
    <tr>
        <td style="padding:4px 0; font-size:13px; color:#888;">Households {{ $neverActivated ? 'Opted In' : 'Affected' }}</td>
    ...
    <tr>
        <td style="padding:4px 0; font-size:13px; color:#888;">Amount to {{ $neverActivated ? 'Activate' : 'Reactivate' }}</td>
    ...
    <tr>
        <td style="padding:4px 0; font-size:13px; color:#888;">{{ $neverActivated ? 'Cancelled On' : 'Paused On' }}</td>
...
<p style="font-size:13px; color:#888; text-align:center; margin:16px 0;">
    @if($neverActivated)
        SOS panic alerts, community safety notifications, and guard communication have
        <strong style="color:#ef4444;">not been enabled</strong> for households on this estate.
        Submit your EFT payment to activate protection.
    @else
...
{{ $neverActivated ? 'Complete Estate Setup' : 'Restore Estate Protection' }}
@endcomponent
