...
<div class="page">
    ...
    <div class="header-inner">
        ...
        <div class="invoice-cell">
            <div class="inv-word">Estate Invoice</div>
            <div class="inv-number">{{ $invoice->invoice_number }}</div>
            <span class="status-pill status-{{ $invoice->status }}">
                {{ strtoupper($invoice->status) }}
            </span><br>
            <span class="estate-badge">Bulk EFT · Estate Billing</span>
        </div>
    </div>
    ...
    <div class="parties">
        ...
        <div class="party-to">
            ...
            <div class="party-detail">
                ...
                @endif
                {{ $invoice->channelSubscription->channel->name }} · Estate Billing Contact
            </div>
        </div>
    </div>
    ...
    <div class="meta-strip">
        ...
        <div class="meta-item">
            ...
            <div class="meta-val">
                {{ $invoice->channelSubscription->current_period_start?->format('d M Y') }}
                –
                {{ $invoice->channelSubscription->current_period_end?->format('d M Y') }}
            </div>
        </div>
        <div class="meta-item">
            <div class="meta-lbl">Payment Method</div>
            <div class="meta-val">{{ strtoupper($invoice->channelSubscriptionPayment->payment_method ?? 'EFT') }}</div>
        </div>
        <div class="meta-item">
            <div class="meta-lbl">Reference</div>
            <div class="meta-val">{{ $invoice->channelSubscriptionPayment->merchant_reference ?? '—' }}</div>
        </div>
        <div class="meta-item">
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>
                    <div class="td-title">Echo Link Estate Bulk Subscription</div>
                    <div class="td-sub">
                        {{ $invoice->channelSubscription->current_period_start?->format('d M Y') }}
                        to
                        {{ $invoice->channelSubscription->current_period_end?->format('d M Y') }}
                ...
                <td>{{ $invoice->channelSubscription->channel->name }}</td>
                <td>{{ $invoice->channelSubscription->household_count }} × R{{ number_format($invoice->channelSubscription->amount_per_household, 2) }}</td>
                <td class="right">R{{ number_format($invoice->total / 100, 2) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="totals-outer">
        <div class="totals-inner">
            <div class="total-row">
                ...
                <div class="total-row-value">R{{ number_format($invoice->subtotal / 100, 2) }}</div>
            </div>
            ...
            <div class="grand-block">
                <div class="grand-left">
                    <div class="grand-lbl">Total Paid</div>
                </div>
    ...
    <div class="footer">
        <div class="footer-left">
            <div class="footer-note">
                ...
                Questions? <strong>billing@echolink.co.za</strong><br>
